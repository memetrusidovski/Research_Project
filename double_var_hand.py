...
for j in [1,2,3,4,5,6]:
    cqm.add_constraint( 
        sum( (x_0[0] [i] * x_0[0] [ (i+j) % n ]) for i in range( n ) ) +
        sum( (x2_0[0] [i] * x2_0[0] [ (i+j) % n ]) for i in range( n ) ) == 2,
        label=f'capacity_bin_{j}1')

for j in [7,8,9,10,11,12,13]:
    cqm.add_constraint( 
        sum( (x_0[1] [i] * x_0[1] [ (i+j) % n ]) for i in range( n ) ) +
        sum( (x2_0[1] [i] * x2_0[1] [ (i+j) % n ]) for i in range( n ) ) == 2,
        label=f'capacity_bin_{j}1')
